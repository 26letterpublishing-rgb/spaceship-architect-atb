...
get('[data-turn]').textContent=person.consoleHold?'HOLDING / 99%':person.delayedAction?.awaitingRoll?'ROLL REQUIRED':ready?'YOUR TURN':person.delayedAction?.sensorOrder?'SCANNING':window.SAConsoleCommon.standby(state,person);
    ...
    view.addEventListener('click',async event=>{
      ...
      if(order&&['hex','life'].includes(order.dataset.order)&&!hexSelected){selecting=order;view.dataset.selectHex='true';get('[data-error]').textContent='Click a hex on the starmap to scan.';get('[data-map]').style.cursor='crosshair';return;}
      get('[data-map]').style.cursor='';
      selecting=null;view.dataset.selectHex='false';
