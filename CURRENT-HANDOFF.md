...
## Ready To Publish: Direct Combat Actions And Full Targeting Families

September 11 #commit implementation and local verification are complete. Read TARGETING-FAMILIES-PASS.md. Numbered Lock-On 1-10 and Rapid Laser 1-5, new tier artwork, root compact combat planners, Defense/AU displays, quiet anchored hits, transparency, veteran Nova and updated Explore hardware are implemented. Extra audit fixes include stale action-response ordering, console preference persistence, quick-close/reopen handling and outer-viewport help. Local verification: 163 unit/HTTP tests; endgame and impaired targeting, explicit/manual damage and misses, family purchases, direct root actions, sensors with GM/two PCs, shields and twelve cockpit orders. FTL Lock-On and Triangulator remain separate add-ons. Publication/hosted verification and cleanup remain: push main, run live verification, stop task-owned processes, send ntfy last. Gold Standard and data/campaigns.json are untouched. Work allowance ends 14:14 UTC. Older completed sections below are historical.
