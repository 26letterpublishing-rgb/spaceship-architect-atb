...
## Reliability Pass 1: Publishing

User authorized the FIRST of two passes with #commit. Reliability/performance implementation and local verification are complete; see RELIABILITY-PASS-1.md. Includes shared boot/layout work, missing NPC prompts and mirrored panel tabs, map synchronization, maintenance/roll result feedback, non-damage dice colors and existing-interface fixes. 170 automated tests and Chrome performance, sensors, cockpit, shields and lasers checks pass. Publish and verify Render before the final ntfy notification. Keep data/campaigns.json and Gold Standard untouched.
