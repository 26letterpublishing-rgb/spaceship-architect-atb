...
get('[data-turn]').textContent=person.consoleHold?'HOLDING / 99%':pending?.awaitingRoll?'ROLL REQUIRED':pending?'FIRE CONTROL INPUT':ready?'YOUR TURN':window.SAConsoleCommon.standby(state,person);
      ...
      const targetId=get('[data-target]').value,spec=window.SAShipWeapons.rollSpec(state,person,{shipId:access.ship.id,targetId}),settings=window.SAShipWeapons.settings(person,access.definition.tier);
