(function(){
  ...
  function open(unit,sicId){
    ...
    function redraw(){
      ...
      get('[data-recovery]').textContent=rest?`${rest.paused||'Restabilizing'} / ${Math.round(rest.progress*100)}% / ${rest.auSpent} of 20 AU supplied`:`Regeneration: 1 HP / ${Number(shield.regenerationSeconds).toFixed(1)} sec`;
    ...
    view.addEventListener('click',async event=>{
      ...
      if(button.dataset.order==='restabilize'&&!host.defaultView.confirm('Restabilize this shield? All crew physically at this shield will stop their ATB until recovery completes. The system needs 20 AU in total and pauses if power runs out.'))return;
