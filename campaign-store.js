...
class CampaignStore {
  ...
  async init() {
    if (this.mode === "postgres") {
      ...
      `);
      // Deployment/version changes must never reset a campaign database.
      return;
    }
    ...
    try {
      ...
      if (!Array.isArray(parsed)) throw new Error("Campaign file must contain an array.");
      for (const campaign of parsed) {
        if (campaign?.code) this.local.set(campaign.code, campaign);
      }
    } catch (error) {
      // Failing startup is preferable to overwriting unreadable saved games.
