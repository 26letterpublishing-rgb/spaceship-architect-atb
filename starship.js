...
{
  const type='lock-on-1',data=window.SAShipMap.definition(type);SIC_CATALOG[type]={...data,category:'lock',shortLabel:data.label,enOutput:0,clearance:0,floorplan:data.image};
  const card=document.createElement('section');card.className='sic-market-item';
  card.innerHTML=`<article class="sic-poker-card" data-sic-card="${type}" tabindex="0" aria-label="Lock-On System 1 details"><header class="sic-poker-heading"><span>Lock-On <small>A-37</small></span><div><h3>Lock-On System 1</h3><strong>Price: 560</strong></div></header><img class="sic-poker-art" src="lock-on-1-card.png" alt="Lock-On targeting computer" loading="lazy"><dl class="sic-poker-stats"><div><dt>Energy Cost</dt><dd>1</dd></div><div><dt>Security Level</dt><dd>1</dd></div><div><dt>Size</dt><dd>1x1</dd></div><div><dt>Skill</dt><dd>Sensor Systems</dd></div><div><dt>Crafting</dt><dd>Paradon, 6 hrs</dd></div></dl><section class="sic-poker-rules"><strong>Lock-On Dice: 2D4</strong><p>Locked targets are hit automatically. Damage still requires a roll.</p><p>Break Lock-On difficulty: 13.<br>4 AU / 12 seconds: maintain one additional target.</p><p>Control from a cockpit or bridge. No local stations.</p></section><footer><span><small>If Impaired</small>Loses current locks each time impairment is taken. Lock-On dice: 2D2.</span><span><small>Damage Threshold</small>3</span></footer></article><button class="sic-purchase-button" data-purchase-sic="${type}" type="button">Purchase</button>`;
  document.querySelector('.sic-card-gallery').append(card);
}
{
