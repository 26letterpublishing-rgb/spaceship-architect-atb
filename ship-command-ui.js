(function() {
  ...
  function conditionalControls(host){
    ...
    element.innerHTML='<summary>Delay an Action</summary><label><input type="checkbox" data-conditional> Arm next order (2 AU)</label><label>Triggering Ship<select data-trigger-target></select></label><label>Trigger<select data-trigger-kind><option value="movement">Any movement</option><option value="distance">Distance traveled</option><option value="hex">Enter hex</option></select></label><label>Distance<input data-trigger-distance type="number" value="1" min="1"></label><div class="command-coordinate"><label>Hex Q<input data-trigger-q type="number" step="1" value="0"></label><label>Hex R<input data-trigger-r type="number" step="1" value="0"></label></div><small>Requires Systems Analysis. One armed order per ship; expires after 12 combat seconds.</small>';
    return element;
  }
