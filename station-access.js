(function(root, factory) {
  ...
  function consoles(room, unit) {
    ...
    return (ship.placements || []).flatMap(p => {
      ...
      if (!online(item) || (!definition.shipControl && !definition.shield && !definition.sensor && !definition.weapon && !definition.lockOn)) return [];
      const remote = seat.cell.sicId !== item.id;
      if (remote && !maps.definition(seat.cell.type).bridge) return [];
      return [{ id: item.id, item, definition, ship: seat.ship, remote, seat, kind: definition.lockOn ? 'lock' : definition.weapon ? 'weapon' : definition.sensor ? 'sensor' : definition.shield ? 'shield' : 'pilot' }];
    });
  }
