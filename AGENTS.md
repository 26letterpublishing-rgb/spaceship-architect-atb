...
- `ship-navigation.js` owns server-authoritative ship orders, AU spending, fractional positions, and inertia. A stationed operational Cockpit 1 pilot with installed thrusters may issue Move Starship. It consumes the pilot turn immediately; the route continues independently. Full Move Speed takes 12 combat seconds. Arrival starts drift at max(0, floor(speed/2)-2), repeatedly decaying every 12 seconds. Mid-route orders replace the old route from the exact current position. AU boosts are paid once per selected thruster/order (Exhaust 4 AU, Ionic 2 AU); no refund for replaced routes. Losing a boosted thruster removes remaining powered boost contribution but retains the frozen base order. Ordinary ship synchronization must preserve server navigation and spent AU.
