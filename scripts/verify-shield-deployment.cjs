...
const files=['index.html','app.js','ship-map-core.js','ship-shields.js','station-access.js','ship-power.js','shield-console-ui.js','ship-navigation.js','ship-navigation-ui.js','ship-navigation-ui.css','starship.html','starship.js','starship.css','character.js','character.css','race-lore-data.js','skill-descriptions.js','skill-descriptions.css'];
async function main(){
  ...
  for(const name of ['shield-1-card','shield-1-floor-plan','shield-console-background','cockpit-2-card','bridge-8-card','bridge-8-floor-plan',...Array.from({length:9},(_,i)=>[`shield-${i+2}-card`,`shield-${i+2}-floor-plan`]).flat()]){
  ...
  try{
    ...
    await page.locator('summary[aria-label="Shield: expand 10 cards"]').click();
    await page.getByRole('dialog',{name:'Shield',exact:true}).waitFor();
    await page.locator('.sic-picker-slot img').evaluateAll(images=>Promise.all(images.map(image=>image.decode())));
    assert.deepEqual(await page.locator('[data-purchase-type]').evaluateAll(nodes=>nodes.map(n=>n.dataset.purchaseType)),Array.from({length:10},(_,i)=>`shield-${i+1}`));
    await page.waitForTimeout(900);await page.screenshot({path:path.join(directory,'hosted-shield-roster.png')});
    assert.deepEqual(errors,[]);
  }finally{await browser.close();}
  console.log('Hosted deployment matches local code and optimized artwork. Bridge and Shield 1-10 pickers render at 1366x768 without page errors. No campaign data changed.');
}
